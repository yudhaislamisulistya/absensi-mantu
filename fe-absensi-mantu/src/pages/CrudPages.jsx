...
export function ClassesPage({ setToast }) {
  ...
  return (
    <CrudLayout eyebrow="DATA MASTER" title="Manajemen Kelas" description="Kelola rombongan belajar, jurusan, dan guru wali." search={search} setSearch={setSearch} addLabel="Tambah kelas" onAdd={() => setForm(empty)} onImport={() => setImportOpen(true)}>
      ...
      <ConfirmDialog open={!!deleting} description={`Kelas “${deleting?.name}” dan seluruh riwayat absensinya akan dihapus permanen. Siswa di kelas ini tetap disimpan sebagai siswa tanpa kelas.`} busy={crud.busy} onClose={() => setDeleting(null)} onConfirm={async () => { if (await crud.remove(deleting.id)) setDeleting(null) }} />
    </CrudLayout>
  )
}
...
export function StudentsPage({ setToast }) {
  ...
  return (
    <CrudLayout eyebrow="DATA MASTER" title="Manajemen Siswa" description="Kelola identitas, kelas, dan status seluruh siswa." search={search} setSearch={setSearch} addLabel="Tambah siswa" onAdd={() => setForm(empty)} onImport={() => setImportOpen(true)}>
      ...
      <ConfirmDialog open={!!deleting} description={`Siswa “${deleting?.name}”, profil wajah, dan seluruh riwayat absensinya akan dihapus permanen.`} busy={crud.busy} onClose={() => setDeleting(null)} onConfirm={async () => { if (await crud.remove(deleting.id)) setDeleting(null) }} />
    </CrudLayout>
  )
}
